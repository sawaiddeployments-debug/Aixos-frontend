...
-- Repair missing columns from older partial deployments.
ALTER TABLE public.sticker_usage_history
...
-- Legacy compatibility: older schema may enforce sticker_code NOT NULL.
-- New flow does not require sticker_code, so make it optional.
ALTER TABLE public.sticker_usage_history
    ADD COLUMN IF NOT EXISTS sticker_code TEXT;

ALTER TABLE public.sticker_usage_history
    ALTER COLUMN sticker_code DROP NOT NULL;

-- Backfill and normalize if used_for existed but had nullable/legacy values.
UPDATE public.sticker_usage_history
SET used_for = lower(trim(coalesce(used_for, 'validation')))
WHERE used_for IS NULL
   OR lower(trim(used_for)) NOT IN ('validation', 'refilled');

-- Tighten used_for constraint safely.
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1
        FROM pg_constraint
        WHERE conname = 'sticker_usage_history_used_for_check'
          AND conrelid = 'public.sticker_usage_history'::regclass
    ) THEN
        ALTER TABLE public.sticker_usage_history
            ADD CONSTRAINT sticker_usage_history_used_for_check
            CHECK (used_for IN ('validation', 'refilled'));
    END IF;
END $$;

-- Ensure unique one-log-per-inquiry behavior.
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1
        FROM pg_constraint
        WHERE conname = 'sticker_usage_history_unique_inquiry'
          AND conrelid = 'public.sticker_usage_history'::regclass
    ) THEN
        ALTER TABLE public.sticker_usage_history
            ADD CONSTRAINT sticker_usage_history_unique_inquiry UNIQUE (inquiry_id);
    END IF;
END $$;
...
-- Recreate RPC expected by frontend.
